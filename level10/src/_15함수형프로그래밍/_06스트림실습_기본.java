...
public class _06스트림실습_기본 {
	...
	public static void main(String[] args) {
		...
		List<Item> copyItemList1 = itemList.stream().distinct().collect(Collectors.toList());
		copyItemList1.forEach(System.out::print);
		
		// 문제 3번 카테고리가 과자인 itemList 출력
		System.out.println("=====[ 3 ]=====");
		Stream<Item> snack = itemList.stream()
				.filter(t -> t.getCategory().equals("과자"));
		snack.forEach(System.out::print);
		
		// 문제 4번 카테고리가 고기인 item 갯수 출력
		System.out.println("=====[ 4 ]=====");
		long meat = itemList.stream()
				.filter(t -> t.getCategory().equals("고기"))
				.count();
		System.out.println("고기개수 = "+meat);
		// 문제 5번 가격이 10000원 이상인 값들의 ArrayList<Item> 만들기
		System.out.println("=====[ 5 ]=====");
		List<Item> copyItemList = itemList.stream()
				.filter(t -> t.getPrice() >= 10000)
				.distinct()
				.collect(Collectors.toList());
		copyItemList.forEach(System.out::print);
		...
		itemList.stream().sorted(Comparator.comparingInt(Item::getItemNo)).forEach(System.out::print);
//		itemList.stream().sorted(item1,item2) -> item1.getItemNo() - item2.getItemNo();
		// 문제 8번 아이템 가격 순으로 정렬
		System.out.println("=====[ 8 ]=====");
		itemList.stream().sorted(Comparator.comparingInt(Item::getPrice)).forEach(System.out::print);
	}
